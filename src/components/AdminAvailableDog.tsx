...
const AdminAvailableDog = async () => {
  const users = await prisma.availableDogs.findMany({});
      ...
      {users.map((availabe, index) => {
        return (
          ...
          >
            ...
            />
            <DeleteDog id={availabe.id} name={availabe.name} />
          </div>
        );
      })}
    </div>
...
export default AdminAvailableDog;
